...
const getGenres = function (gerneList) {
  const newGenreList = [];

  for (const { name } of gerneList) newGenreList.push(name);
...
const filterVideos = function (videoList) {
  return videoList.filter(
    ({ type, site }) =>
      (type === "Trailer" || type === "Teaser" || type === "Featurette") &&
      site === "YouTube"
  );
};

fetchDataFromServer(
  `https://api.themoviedb.org/3/movie/${movieId}?api_key=${api_key}&append_to_response=casts,videos,releases,images`,
  function (movie) {
    const {
      backdrop_path,
      poster_path,
 ...
 <div class="backdrop-image" style="background-image: url('${imageBasrURL}${
      "w1280" || "original"
    }
 ${backdrop_path || poster_path}')"></div>

            <figure class="poster-box movie-poster">
                <img src="${imageBasrURL}w342${poster_path}" alt="${title} poster" class="img-cover">
            </figure>
