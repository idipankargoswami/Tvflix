...
// import all componenet and function
console.log("index");
import { sidebar } from "./sidebar.js";
import { api_key, imageBasrURL, fetchDataFromServer } from "./api.js";
...
const gerneList = {
  // create gerne string from genre_id eg :[23, 43] -> "Action, Romance"

  asString(genreIdList) {
    console.log("bapan", genreIdList);

    let newGenreList = [];
    for (const gerneId of genreIdList) {
      gerneList[gerneId] && newGenreList.push(gerneList[gerneId]); //this = gerneList
    }
...
<img src="${imageBasrURL}w1280${backdrop_path}" alt="${title}" class="img-cover" loading=${
      index === 0 ? "eager" : "lazy"
    }>
<div class="banner-content">
  <h2 class="heading">${title}</h2>
  <div class="meta-item card-bedge">${vote_average.toFixed(1)}</div>
  <div class="meta-list">
    <div class="meta-item">${releaseDateFormatted}</div>
  </div>
  <p class="genre">${gerneList.asString(genre_ids)}</p>
...
<img src="${imageBasrURL}w154${poster_path}" alt="Slide to${title}" loading="lazy" draggable="false"
  class="img-cover" />
`;
